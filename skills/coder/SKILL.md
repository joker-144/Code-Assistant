...
可运行 `scripts/analyzer.js` 自动完成分析：`node scripts/analyzer.js [项目路径]`
...
### 代码质量
- 所有异步操作必须有 try-catch，错误信息清晰
- 使用统一日志库，禁止 console.log（开发调试除外）
- 复杂逻辑必须注释，解释"为什么"而非"是什么"

### 安全规范
- 禁止硬编码 API keys、密码，必须使用环境变量
- 所有用户输入必须验证和清理
- SQL查询使用参数化查询或 ORM
...
- 完整索引：`rules/index.json`
- 分析脚本：`scripts/analyzer.js`
