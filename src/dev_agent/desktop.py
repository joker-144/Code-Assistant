"""
Desktop launcher for dev-agent.
...
def _is_electron_available() -> bool:
    ...
    electron_indicators = [
        project_root / "dist-electron" / "win-unpacked" / "DevAgent.exe",
        project_root / "dist-electron" / "mac" / "DevAgent.app",
        project_root / "dist-electron" / "mac-arm64" / "DevAgent.app",
        project_root / "dist-electron" / "linux-unpacked" / "dev-agent",
...
def _launch_electron():
    ...
    if not web_dir.is_dir():
        print("[DevAgent] Web directory not found.", file=sys.stderr)
        return False
    ...
    if dist_electron.is_dir():
        if sys.platform == "win32":
            exe = dist_electron / "win-unpacked" / "DevAgent.exe"
            if exe.exists():
                subprocess.Popen([str(exe)], cwd=str(web_dir))
                return True
        elif sys.platform == "darwin":
            for arch in ["mac", "mac-arm64"]:
                app = dist_electron / arch / "DevAgent.app"
                if app.is_dir():
                    subprocess.Popen(["open", str(app)], cwd=str(web_dir))
                    return True
        else:
            exe = dist_electron / "linux-unpacked" / "dev-agent"
            if exe.exists():
                subprocess.Popen([str(exe)], cwd=str(web_dir))
                return True
...
def open_browser(url: str = DEFAULT_URL):
    ...
    """
    print(f"[DevAgent] Opening browser at {url}")
    webbrowser.open(url)
...
def launch_desktop():
    ...
    if _is_electron_available():
        print("[DevAgent] Electron detected, launching native window...")
        if _launch_electron():
            return

    # Fallback to browser
    print("[DevAgent] No Electron found, launching in browser...")
    open_browser()
