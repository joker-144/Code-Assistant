...
Embedding 模型: BAAI/bge-small-zh-v1.5（512 维，~100MB，支持中英文）
首次使用自动从 HuggingFace 下载，之后完全离线运行。
"""
...
class ProjectIndex:
    ...
    def __init__(self, workspace: Path, store: Optional[MemoryStore] = None):
        ...
        self._embedder = None  # 延迟初始化（首次使用时加载模型）

    @property
    def embedder(self):
        """延迟加载 Embedding 模型（首次使用时下载，约 100MB）"""
    ...
    def _walk_source_files(self):
        """遍历项目中所有可索引的源文件"""
        for root, dirs, files in self.workspace.walk():
            # 过滤排除目录
    ...
    def search(self, query: str, top_k: int = 5) -> list[CodeChunk]:
        ...
        # 2. 加载所有向量做余弦相似度搜索
        rows = self.store.load_all_embeddings()
        if not rows:
            return []
